...
import { createPromoCode } from "../api/promo.api";
...
export const PromoAddModal = ({ onClose }: PromoAddModalProps) => {
  ...
  const handleSubmit = async () => {
    if (!form.promoName.trim()) {
      showToast("error", "Promo name is required.");
      return;
    }
    if (!form.type) {
    ...
    if (
      ...
      showToast("error", "Select at least one merchandise or category.");
      return;
    }
